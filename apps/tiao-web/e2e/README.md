...
**Não rode durante a pesagem.** A suíte escreve três animais de mentira
(`e2e-901`, `e2e-902`, `e2e-903`) e as pesagens do dia deles, e apaga tudo no
fim. Se ela for interrompida no meio, essas linhas ficam para trás e aparecem na
caderneta do Seu Jader até alguém apagar à mão:
...
`TIAO_RESOURCE_ID` (`285`, o id do recurso no Pangolin) e `TIAO_ESPERA_LIMITE`
(quantos segundos aceitar esperar o limitador de PIN; padrão `900`).

## O limitador de PIN

O Pangolin só aceita **15 tentativas de PIN a cada 15 minutos** por cliente, e
depois disso responde `429` com um `Retry-After`. Uma rodada inteira gasta duas
tentativas — a errada e a certa —, mas quem andou testando o portão à mão pouco
antes pode ter esgotado a janela. A suíte trata o `429` como "ainda não me
responderam", não como resposta: ela espera o `Retry-After` e tenta de novo, até
`TIAO_ESPERA_LIMITE`. Se aceitasse o `429` como resposta,
`test_pin_errado_nao_abre` passaria sem provar nada.
